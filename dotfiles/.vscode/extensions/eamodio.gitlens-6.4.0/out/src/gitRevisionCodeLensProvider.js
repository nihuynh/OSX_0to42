...
class GitDiffWithWorkingCodeLens extends vscode_1.CodeLens {
    constructor(fileName, commit, range) {
        super(range);
...
class GitRevisionCodeLensProvider {
    ...
    provideCodeLenses(document, token) {
        return __awaiter(this, void 0, void 0, function* () {
            const gitUri = gitService_1.GitUri.fromRevisionUri(document.uri);
            const lenses = [];
            const commit = yield this.git.getLogCommit(gitUri.repoPath, gitUri.fsPath, gitUri.sha, { firstIfMissing: true, previous: true });
            if (commit === undefined)
                return lenses;
            if (commit.previousSha) {
                lenses.push(new GitDiffWithPreviousCodeLens(commit.previousUri.fsPath, commit, new vscode_1.Range(0, 0, 0, 1)));
            }
            lenses.push(new GitDiffWithWorkingCodeLens(commit.uri.fsPath, commit, new vscode_1.Range(0, 1, 0, 2)));
            return lenses;
        });
    }
