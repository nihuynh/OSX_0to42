...
class MessageQuickPickItem extends CommandQuickPickItem {
    constructor(message) {
        super({ label: message, description: '' });
    }
}
exports.MessageQuickPickItem = MessageQuickPickItem;
...
class CommitQuickPickItem {
    constructor(commit) {
        ...
        if (commit.isStash) {
            this.label = message;
            this.description = '';
            this.detail = `${constants_1.GlyphChars.Space} ${commit.stashName || commit.shortSha} ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.fromNow()} ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.getDiffStatus()}`;
        }
        else {
            this.label = message;
            this.description = `${system_1.Strings.pad('$(git-commit)', 1, 1)} ${commit.shortSha}`;
            this.detail = `${constants_1.GlyphChars.Space} ${commit.author}, ${commit.fromNow()}${commit.isFile ? '' : ` ${system_1.Strings.pad(constants_1.GlyphChars.Dot, 1, 1)} ${commit.getDiffStatus()}`}`;
        }
    }
}
