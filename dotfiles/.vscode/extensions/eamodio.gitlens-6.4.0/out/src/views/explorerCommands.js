...
class ExplorerCommands extends vscode_1.Disposable {
    ...
    openChangesWithWorking(node) {
        ...
        return vscode_1.commands.executeCommand(commands_1.Commands.DiffWithWorking, node.commit.toGitUri(), args);
    }
    openFile(node) {
    ...
    openFileRevision(node, options = { showOptions: { preserveFocus: true, preview: false } }) {
        const uri = options.uri || (node.commit.status === 'D'
            ? gitService_1.GitUri.toRevisionUri(node.commit.previousSha, node.commit.previousUri.fsPath, node.commit.repoPath)
            : gitService_1.GitUri.toRevisionUri(node.uri));
        return commands_1.openEditor(uri, options.showOptions || { preserveFocus: true, preview: false });
    }
    ...
    openChangedFiles(node, options = { preserveFocus: false, preview: false }) {
        return __awaiter(this, void 0, void 0, function* () {
            const repoPath = node.commit.repoPath;
            const uris = system_1.Arrays.filterMap(node.commit.fileStatuses, f => gitService_1.GitUri.fromFileStatus(f, repoPath));
            for (const uri of uris) {
    ...
    openChangedFileRevisions(node, options = { preserveFocus: false, preview: false }) {
        return __awaiter(this, void 0, void 0, function* () {
            const uris = system_1.Arrays.filterMap(node.commit.fileStatuses, f => gitService_1.GitUri.toRevisionUri(f.status === 'D' ? node.commit.previousFileSha : node.commit.sha, f, node.commit.repoPath));
            for (const uri of uris) {
                yield commands_1.openEditor(uri, options);
            }
        });
    ...
    openFileRevisionInRemote(node) {
        return __awaiter(this, void 0, void 0, function* () {
            return vscode_1.commands.executeCommand(commands_1.Commands.OpenFileInRemote, node.commit.toGitUri(node.commit.status === 'D'), { range: false });
        });
    }
