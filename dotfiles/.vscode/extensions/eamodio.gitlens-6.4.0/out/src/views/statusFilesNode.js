...
class StatusFilesNode extends explorerNode_1.ExplorerNode {
    constructor(status, range, explorer, branch) {
        super(gitService_1.GitUri.fromRepoPath(status.repoPath));
        this.status = status;
        this.range = range;
            ...
            if (this.status.files.length !== 0 && this.includeWorkingTree) {
                statuses.splice(0, 0, ...system_1.Iterables.flatMap(this.status.files, s => {
                    if (s.workTreeStatus !== undefined && s.indexStatus !== undefined) {
                        ...
                        return [
                            Object.assign({}, s, { status: s.status, commit: new gitService_1.GitLogCommit(gitService_1.GitCommitType.File, repoPath, gitService_1.GitService.uncommittedSha, 'You', new Date(), '', s.fileName, [s], s.status, s.originalFileName, gitService_1.GitService.stagedUncommittedSha, s.fileName) }),
                            Object.assign({}, s, { status: s.status, commit: new gitService_1.GitLogCommit(gitService_1.GitCommitType.File, repoPath, gitService_1.GitService.stagedUncommittedSha, 'You', older, '', s.fileName, [s], s.status, s.originalFileName, 'HEAD', s.fileName) })
                        ];
                    }
                    else if (s.indexStatus !== undefined) {
                        return [
                            Object.assign({}, s, { status: s.status, commit: new gitService_1.GitLogCommit(gitService_1.GitCommitType.File, repoPath, gitService_1.GitService.stagedUncommittedSha, 'You', new Date(), '', s.fileName, [s], s.status, s.originalFileName, 'HEAD', s.fileName) })
                        ];
                    }
                    else {
                        return [
                            Object.assign({}, s, { status: s.status, commit: new gitService_1.GitLogCommit(gitService_1.GitCommitType.File, repoPath, gitService_1.GitService.uncommittedSha, 'You', new Date(), '', s.fileName, [s], s.status, s.originalFileName, 'HEAD', s.fileName) })
                        ];
                    }
                }));
            }
