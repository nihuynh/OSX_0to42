    ...
    execute(editor, uri, args = {}) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (args.repoPath === undefined || args.lhs === undefined || args.rhs === undefined)
                return undefined;
            try {
                if (gitService_1.GitService.isResolveRequired(args.lhs.sha)) {
                    args.lhs.sha = yield this.git.resolveReference(args.repoPath, args.lhs.sha, args.lhs.uri);
                }
                if (gitService_1.GitService.isResolveRequired(args.rhs.sha)) {
                    args.rhs.sha = yield this.git.resolveReference(args.repoPath, args.rhs.sha, args.rhs.uri);
                }
                ...
                let rhsPrefix = '';
                if (rhs === undefined) {
                    rhsPrefix = gitService_1.GitService.isUncommitted(args.rhs.sha)
                        ? ' (deleted)'
                        : 'deleted in ';
                }
                ...
                if (args.lhs.title === undefined && args.lhs.sha !== gitService_1.GitService.deletedSha && (lhs !== undefined || lhsPrefix !== '')) {
                ...
                return yield vscode_1.commands.executeCommand(constants_1.BuiltInCommands.Diff, lhs === undefined
                    ? gitService_1.GitUri.toRevisionUri(gitService_1.GitService.deletedSha, args.lhs.uri.fsPath, args.repoPath)
                    : vscode_1.Uri.file(lhs), rhs === undefined
                    ? gitService_1.GitUri.toRevisionUri(gitService_1.GitService.deletedSha, args.rhs.uri.fsPath, args.repoPath)
