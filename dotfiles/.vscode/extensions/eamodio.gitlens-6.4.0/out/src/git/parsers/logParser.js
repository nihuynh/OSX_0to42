...
class GitLogParser {
    ...
    static parseEntry(entry, commit, type, repoPath, relativeFileName, commits, authors, recentCommit) {
        if (commit === undefined) {
            ...
            if (type === git_1.GitCommitType.File) {
                entry.fileStatuses = [{ status: entry.status, fileName: relativeFileName, originalFileName: originalFileName }];
            }
            commit = new git_1.GitLogCommit(type, repoPath, entry.sha, entry.author, new Date(entry.authorDate * 1000), entry.summary, relativeFileName, entry.fileStatuses || [], entry.status, originalFileName, undefined, undefined, entry.parentShas);
