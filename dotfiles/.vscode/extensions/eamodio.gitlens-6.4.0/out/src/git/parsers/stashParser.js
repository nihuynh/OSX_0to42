...
class GitStashParser {
    static parse(data, repoPath) {
        ...
        for (let i = 0, len = entries.length; i < len; i++) {
            ...
            if (commit === undefined) {
                commit = new git_1.GitStashCommit(git_1.GitCommitType.Stash, entry.stashName, repoPath, entry.sha, new Date(entry.date * 1000), entry.summary, entry.fileNames, entry.fileStatuses || []);
            ...
            switch (lineParts[0]) {
                ...
                case 'filename':
                    ...
                    if (entry.fileStatuses) {
                        entry.fileNames = system_1.Arrays.filterMap(entry.fileStatuses, f => !!f.fileName ? f.fileName : undefined).join(', ');
                    }
                    entries.push(entry);
                    entry = undefined;
                    break;
